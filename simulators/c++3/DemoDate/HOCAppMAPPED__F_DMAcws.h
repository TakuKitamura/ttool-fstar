#ifndef HOCAPPMAPPED__F_DMACWS__H
#define HOCAPPMAPPED__F_DMACWS__H
...
class HOCAppMAPPED__F_DMAcws: public TMLTask {
    private:
    // Attributes
    ParamType size;
    ParamType rnd__0;
    ...
    TMLWaitCommand _wait302;
    TMLRequestCommand _request299;
    TMLSendCommand _send301;
    TMLStopCommand _stop300;
    
    Parameter* _wait302_func(Parameter* ioParam);
    Parameter* _request299_func(Parameter* ioParam);
    Parameter* _send301_func(Parameter* ioParam);
    
    public:
    friend bool condFunc(TMLTask* _ioTask_);
    friend class CurrentComponents;
    HOCAppMAPPED__F_DMAcws(ID iID, Priority iPriority, std::string iName, CPU** iCPUs, unsigned int iNumOfCPUs
