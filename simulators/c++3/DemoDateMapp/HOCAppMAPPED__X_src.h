#ifndef HOCAPPMAPPED__X_SRC__H
#define HOCAPPMAPPED__X_SRC__H
...
class HOCAppMAPPED__X_src: public TMLTask {
    private:
    // Attributes
    ParamType r_size;
    ParamType arg1__req;
    ...
    TMLActionCommand _action311;
    TMLExeciCommand _execi308;
    TMLWriteCommand _write310;
    
    Parameter* waitOnRequest_func(Parameter* ioParam);
    TMLLength _execi308_func();
    TMLLength _write310_func();
    void _action311_func();
    
    public:
    friend bool condFunc(TMLTask* _ioTask_);
    friend class CurrentComponents;
    HOCAppMAPPED__X_src(ID iID, Priority iPriority, std::string iName, CPU** iCPUs, unsigned int iNumOfCPUs
