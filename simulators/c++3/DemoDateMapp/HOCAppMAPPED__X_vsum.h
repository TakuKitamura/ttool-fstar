#ifndef HOCAPPMAPPED__X_VSUM__H
#define HOCAPPMAPPED__X_VSUM__H
...
class HOCAppMAPPED__X_vsum: public TMLTask {
    private:
    // Attributes
    ParamType size;
    ParamType arg1__req;
    ...
    TMLActionCommand _action288;
    TMLReadCommand _read287;
    TMLExeciCommand _execi284;
    TMLWriteCommand _write286;
    
    Parameter* waitOnRequest_func(Parameter* ioParam);
    TMLLength _read287_func();
    TMLLength _execi284_func();
    TMLLength _write286_func();
    void _action288_func();
    
    public:
    friend bool condFunc(TMLTask* _ioTask_);
    friend class CurrentComponents;
    HOCAppMAPPED__X_vsum(ID iID, Priority iPriority, std::string iName, CPU** iCPUs, unsigned int iNumOfCPUs
