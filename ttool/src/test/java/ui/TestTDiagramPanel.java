...
public class TestTDiagramPanel extends AbstractUITest {
  ...
  public void testMoveCompositeComponentAfterZoom() {
    ...
    final Point expectedPos = new Point(compositeComponent.getX() + TDiagramPanel.MOVE_SPEED,
        compositeComponent.getY());
  ...
  private void checkResults(final Point expectedPos, final TGComponent component) {
    assertTrue("X position is " + component.getX() + " instead of " + expectedPos.x + ".",
        component.getX() == expectedPos.x);
    assertTrue("Y position is " + component.getY() + " instead of " + expectedPos.y + ".",
        component.getY() == expectedPos.y);
  }
}
