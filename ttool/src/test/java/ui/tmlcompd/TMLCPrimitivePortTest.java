...
public class TMLCPrimitivePortTest extends AbstractUITest {
    ...
    public void setUp() throws Exception {
        ...
        if (diagramPanel != null && primitiveComponent1 != null && primitiveComponent2 != null) {
            eventPortOut = new TMLCChannelOutPort(509, 672, -13, 187, -13, 583, true, primitiveComponent1,
                    diagramPanel);
            ...
            channelPortOut = new TMLCChannelOutPort(509, 625, -13, 187, -13, 583, true, primitiveComponent1,
                    diagramPanel);
            channelPortOut.isOrigin = true;
            channelPortOut.typep = 0;
            channelPortIn = new TMLCChannelOutPort(708, 630, -13, 187, -13, 578, true, primitiveComponent2,
                    diagramPanel);
            channelPortIn.isOrigin = false;
            channelPortIn.typep = 0;

            requestPortOut = new TMLCChannelOutPort(509, 730, -13, 187, -13, 583, true, primitiveComponent1,
                    diagramPanel);
            requestPortOut.isOrigin = true;
            requestPortOut.typep = 2;
            requestPortIn = new TMLCChannelOutPort(708, 747, -13, 187, -13, 578, true, primitiveComponent2,
                    diagramPanel);
    ...
    public void testEventPortName() throws Exception {
        assertTrue("Test : Port name in use",
                ((TMLComponentTaskDiagramPanel) diagramPanel).namePrimitivePortInUse(eventPortIn, EVENT_IN));
        assertTrue("Test : Port name in use",
                ((TMLComponentTaskDiagramPanel) diagramPanel).namePrimitivePortInUse(eventPortOut, EVENT_OUT));
        assertFalse("Test : Port name in use",
                ((TMLComponentTaskDiagramPanel) diagramPanel).namePrimitivePortInUse(eventPortOut, EVENT_IN));
        assertFalse("Test : Port name in use",
                ((TMLComponentTaskDiagramPanel) diagramPanel).namePrimitivePortInUse(eventPortIn, EVENT_OUT));
    }

    @Test
    public void testChannelPortName() throws Exception {
        assertTrue("Test : Port name in use",
                ((TMLComponentTaskDiagramPanel) diagramPanel).namePrimitivePortInUse(channelPortIn, CHANNEL_IN));
        assertTrue("Test : Port name in use",
                ((TMLComponentTaskDiagramPanel) diagramPanel).namePrimitivePortInUse(channelPortOut, CHANNEL_OUT));
        assertFalse("Test : Port name in use",
                ((TMLComponentTaskDiagramPanel) diagramPanel).namePrimitivePortInUse(channelPortOut, CHANNEL_IN));
        assertFalse("Test : Port name in use",
                ((TMLComponentTaskDiagramPanel) diagramPanel).namePrimitivePortInUse(channelPortIn, CHANNEL_OUT));
    }

    @Test
    public void tesRequestPortName() throws Exception {
        // assertTrue("Test : Port name in use",((TMLComponentTaskDiagramPanel)
        // diagramPanel).namePrimitivePortInUse(requestPortIn,EVENT_IN));
        // assertTrue("Test : Port name in use",((TMLComponentTaskDiagramPanel)
        // diagramPanel).namePrimitivePortInUse(requestPortOut,EVENT_OUT));
        assertFalse("Test : Port name in use",
                ((TMLComponentTaskDiagramPanel) diagramPanel).namePrimitivePortInUse(requestPortOut, REQUEST_IN));
        assertFalse("Test : Port name in use",
                ((TMLComponentTaskDiagramPanel) diagramPanel).namePrimitivePortInUse(requestPortIn, REQUEST_OUT));
    }
    ...
    public void testIsAValidName() throws Exception {
        ...
        assertFalse("name with specialized character",
                TAttribute.isAValidPortName(INVALID_NAME_4, true, true, true, true));
        ...
        assertFalse("fault at one of the port names",
                TAttribute.isAValidPortName(INVALID_NAME_10, true, true, true, true));
    }
