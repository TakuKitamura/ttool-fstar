...
public class TimelineDiagramTest extends AbstractUITest {
  ...
  final static String EXPECTED_FILE_GENERATED_TIMELINE = getBaseResourcesDir()
      + "tmltranslator/expected/expected_get_generated_timeline.txt";
  ...
  public void testCompareTimelineGeneratedContent() throws Exception {
    for (int i = 0; i < MODELS_TIMELINE.length; i++) {
      ...
      Thread.sleep(1000);
      // Connects to the simulator, incase of using terminal: "./run.x -server" to
      // start server and "nc localhost 3490" to connect to server
