  ...
  public void testCPUShowTraceOnDoubleClick() throws Exception {
    for (int i = 0; i < MODELS_CPU_SHOWTRACE.length; i++) {
      ...
      if (jfis != null) {
        ...
        TraceManager
            .addDev("HashCode  = " + hashOK + " and Busy mode = " + ((jfis.getBusyMode() == 1) ? "READY" : "BUSY"));
        ...
        for (TGComponent tg : currTdp.getComponentList()) {
          ...
          // get the transaction list of each CPUs on the panel, if the trans size > 0
          // then there will be a trace shown on double click
          if (tg instanceof TMLArchiCPUNode) {
            ...
            // mainGUI.getTransactions(_ID) is synchronized function, so we need to wait
            // until data is filled.
            ...
            if (ts != null)
              TraceManager.addDev("Device " + _ID + " has trans size = " + ts.size());
