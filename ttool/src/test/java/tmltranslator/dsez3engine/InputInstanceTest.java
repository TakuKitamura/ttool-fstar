...
public class InputInstanceTest {
    ...
    public void findOptimizedMapping() {
        ...
        Log.close();
        /*
         * if (Log.isOpen()) TraceManager.addDev("Log is still open!"); } catch
         * (Z3Exception ex) { TraceManager.addDev("Z3 Managed Exception: " +
         * ex.getMessage()); TraceManager.addDev("Stack trace: "); ex.printStackTrace(
         * System.out); } catch (Exception ex) {
         * TraceManager.addDev("Unknown Exception: " + ex.getMessage());
         * TraceManager.addDev("Stack trace: "); ex.printStackTrace(System.out); }
         */
    ...
    public void getLocalMemoryOfHwExecutionNode() {
        ...
        assertTrue("comparing between the expected local memory for main_CPU and the memory found",
                inputInstance.getLocalMemoryOfHwExecutionNode(
                        inputInstance.getArchitecture().getHwNodeByName("MainCPU")) == output1);
        assertTrue("comparing between the expected local memory for DSP and the memory found", inputInstance
                .getLocalMemoryOfHwExecutionNode(inputInstance.getArchitecture().getHwNodeByName("dsp")) == output2);
    ...
    public void getWCET() {
        ...
        assertEquals("comparing between the expected WCET of task A on main_CPU and the computed value",
                expectedWcetA_CPU, inputInstance.getWCET(tempTasks.get(0),
                        (HwExecutionNode) inputInstance.getArchitecture().getHwNodeByName("MainCPU")));
        assertEquals("comparing between the expected WCET of task A on DSP and the computed value", expectedWcetA_dsp,
                inputInstance.getWCET(tempTasks.get(0),
                        (HwExecutionNode) inputInstance.getArchitecture().getHwNodeByName("dsp")));

    }

    @Test
    public void getFinalTask() {
        assertEquals("checking the last task", inputInstance.getModeling().getTMLTaskByName("task__E"),
                inputInstance.getFinalTask(inputInstance.getModeling()));
