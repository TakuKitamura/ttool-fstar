...
public class InputInstance {
  ...
  public List<HwExecutionNode> getFeasibleCPUs(TMLTask tmlTask) {
    ...
    for (HwNode hwNode : architecture.getHwNodes()) {
      if (hwNode instanceof HwExecutionNode) {
        ...
        if ((((HwExecutionNode) hwNode).supportOperation(tmlTask.getOperation()))
            || (((HwExecutionNode) hwNode).getOperation().equals(" ")))
  ...
  public int getBufferIn(TMLTask tmlTask) {
    ...
    // TODO if tmlReadChannel.getNbOfSamples() does not return int msg: please enter
  ...
  // should work for cases where each processing unit is equipped with a unique
  // local memory connected directly through a bus
