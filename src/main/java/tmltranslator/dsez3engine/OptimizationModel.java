...
public class OptimizationModel {
    ...
    public OptimizationResult findFeasibleMapping(Context ctx) {
        ...
        // Definition of Xtp as a matrix. Each cell corresponds to a combination of a
        // task and a processor.
        IntExpr[][] X = new IntExpr[inputInstance.getModeling().getTasks().size()][inputInstance.getArchitecture()
                .getCPUs().size()]; // getCPUs returns only HwCPU elements

        // TODO paths

        // Definition of Ycw as a matrix. Each cell corresponds to a combination of a
        // channel and a path.
        // IntExpr[][] Y = new
        // IntExpr[inputInstance.getModeling().getChannels().size()][tmlcpSequenceDiagrams.size()];
        ...
        BoolExpr[][] c_bound_x = new BoolExpr[inputInstance.getModeling().getTasks().size()][inputInstance
                .getArchitecture().getCPUs().size()];
        ...
        for (Object tmlTask : inputInstance.getModeling().getTasks()) {
            ...
            for (HwNode hwNode : inputInstance.getArchitecture().getCPUs()) {
                ...
                // Constraints: 0 <= Xtp <= 1
                c_bound_x[t][p] = ctx.mkAnd(ctx.mkLe(ctx.mkInt(0), X[t][p]), ctx.mkLe(X[t][p], ctx.mkInt(1)));
        ...
        BoolExpr[][] c_mem = new BoolExpr[inputInstance.getArchitecture().getNbOfCPU()][inputInstance.getModeling()
                .getTasks().size()];
        ...
        for (HwNode hwNode : inputInstance.getArchitecture().getCPUs()) {
            ...
            for (Object tmlTask : inputInstance.getModeling().getTasks()) {
                ...
                IntExpr bin_plus_bout = ctx
                        .mkInt(inputInstance.getBufferIn(taskCast) + inputInstance.getBufferOut(taskCast));
        ...
        /*
         * //At least one route Constraint: ∀c, (t1,t2) in c; X t1p1 + X t2p2 − r p1p2 ≤
         * 1 TraceManager.addDev("\nAt least one route constraints (6)"); BoolExpr[]
         * c_route = new BoolExpr[inputInstance.getModeling().getChannels().size()];
         * 
         * IntExpr[][] r = new
         * IntExpr[inputInstance.getArchitecture().getCPUs().size()][inputInstance.
         * getArchitecture().getCPUs().size()];
         * 
         * for (TMLChannel channel : (List<TMLChannel>)
         * inputInstance.getModeling().getChannels()) {
         * 
         * int c = inputInstance.getModeling().getChannels().indexOf(channel); //for
         * each channel get producer and consumer TMLTask producer =
         * channel.getOriginTask(); int prodIndex =
         * inputInstance.getModeling().getTasks().indexOf(producer);
         * 
         * TMLTask consumer = channel.getDestinationTask(); int consIndex =
         * inputInstance.getModeling().getTasks().indexOf(consumer);
         * 
         * for (HwNode source : inputInstance.getArchitecture().getCPUs()) { int src =
         * inputInstance.getArchitecture().getCPUs().indexOf(source);
         * 
         * for (HwNode destination : inputInstance.getArchitecture().getCPUs()) { int
         * dst = inputInstance.getArchitecture().getCPUs().indexOf(destination); //for
         * each couple of processors TODO calculate rp1p2 r[src][dst] = ctx.mkInt(1);
        ...
        for (Object channel : inputInstance.getModeling().getChannels()) {
            ...
            TraceManager
                    .addDev("prodIndex" + prodIndex + "producer " + producer + inputInstance.getModeling().getTasks());
            ...
            for (HwExecutionNode hwNode : inputInstance.getFeasibleCPUs(producer)) {
                ...
                wcetProducer = ctx.mkAdd(wcetProducer,
                        ctx.mkMul(X[prodIndex][p], ctx.mkInt(inputInstance.getWCET(producer, hwNode))));
        ...
        // ∀p∀t i ∀t j 6 = t i , ¬((X t i p = 1) ∧ (X t j p = 1)) ∨ ((τ s (t j ) ≥ τ e
        // (t i )) ∨ (τ s (t i ) ≥ τ e (t j )))
        // Scheduling: One single task running on CPU ∀t
        // TraceManager.addDev("\n Same resource/no time overlap Constraints (12)");
        BoolExpr c_monoTask[][][] = new BoolExpr[inputInstance.getArchitecture().getCPUs().size()][inputInstance
                .getModeling().getTasks().size()][inputInstance.getModeling().getTasks().size()];
        ...
        for (HwNode hwNode : inputInstance.getArchitecture().getCPUs()) {
            ...
            for (Object taski : inputInstance.getModeling().getTasks()) {
                ...
                for (HwExecutionNode ihwNode : inputInstance.getFeasibleCPUs(taskiCast)) {
                    ...
                    wceti = ctx.mkAdd(wceti,
                            ctx.mkMul(X[ti][ip], ctx.mkInt(inputInstance.getWCET(taskiCast, ihwNode))));
                ...
                for (Object taskj : inputInstance.getModeling().getTasks()) {
                    ...
                    if (tj != ti) {
                        ...
                        for (HwExecutionNode jhwNode : inputInstance.getFeasibleCPUs(taskjCast)) {
                            ...
                            wcetj = ctx.mkAdd(wcetj,
                                    ctx.mkMul(X[tj][jp], ctx.mkInt(inputInstance.getWCET(taskjCast, jhwNode))));
        ...
        if (s.check() == Status.SATISFIABLE) {
            Model m = s.getModel();
            Expr[][] optimized_result_X = new Expr[inputInstance.getModeling().getTasks().size()][inputInstance
                    .getArchitecture().getCPUs().size()];
            ...
            for (Object task : inputInstance.getModeling().getTasks()) {
                ...
                for (HwNode hwNode : inputInstance.getArchitecture().getCPUs()) {
                    ...
                    TraceManager.addDev(
                            "X[" + taskCast.getName() + "][" + hwNode.getName() + "] = " + optimized_result_X[t][p]);
    ...
    public OptimizationResult findOptimizedMapping(Context ctx) {
        ...
        IntExpr[][] Y1 = new IntExpr[inputInstance.getModeling().getChannels().size()][inputInstance.getArchitecture()
                .getCPUs().size()];
        ...
        BoolExpr[][] c_bound_y1 = new BoolExpr[inputInstance.getModeling().getChannels().size()][inputInstance
                .getArchitecture().getCPUs().size()];
        ...
        for (Object channel : inputInstance.getModeling().getChannels()) {
            ...
            for (HwNode hwNode : inputInstance.getArchitecture().getCPUs()) {
                ...
                // Constraints: 0 <= Y1 <= 1
                c_bound_y1[c][p] = ctx.mkAnd(ctx.mkLe(ctx.mkInt(0), Y1[c][p]), ctx.mkLe(Y1[c][p], ctx.mkInt(1)));
        ...
        for (HwNode hwNode : inputInstance.getArchitecture().getCPUs()) {
            ...
            IntExpr mem = ctx.mkInt(inputInstance.getLocalMemoryOfHwExecutionNode(hwNode).memorySize);
            // TraceManager.addDev("local memory of " + hwNode.getName() + " is :" +
            // inputInstance.getLocalMemoryOfHwExecutionNode(hwNode).getName());
        ...
        int finalTaskIndex = inputInstance.getModeling().getTasks()
                .indexOf(inputInstance.getFinalTask(inputInstance.getModeling()));
        ArithExpr wcet_last = ctx.mkInt(0);
        int l = 0;
        for (HwExecutionNode hwNode : inputInstance
                .getFeasibleCPUs(inputInstance.getFinalTask(inputInstance.getModeling()))) {
            int p = inputInstance.getArchitecture().getCPUs().indexOf(hwNode);
            wcet_last = ctx.mkAdd(wcet_last,
                    ctx.mkMul(X[finalTaskIndex][p], ctx.mkInt(inputInstance.getWCET(finalTask, hwNode))));
            l += 1;
        ...
        if (opt.Check() == Status.SATISFIABLE) {
            ...
            Expr[][] optimized_result_X = new Expr[inputInstance.getModeling().getTasks().size()][inputInstance
                    .getArchitecture().getCPUs().size()];
            Expr[] optimized_result_start = new Expr[inputInstance.getModeling().getTasks().size()];
            Expr[][] optimized_result_Y1 = new Expr[inputInstance.getModeling().getChannels().size()][inputInstance
                    .getArchitecture().getCPUs().size()];
                ...
                for (HwNode hwNode : inputInstance.getArchitecture().getCPUs()) {
                    ...
                    if (Integer.parseInt(optimized_result_X[t][p].toString()) == 1) {
                        ...
                        if (!taskCast.getReadChannels().isEmpty())
                            tmlMapping.addCommToHwCommNode(taskCast.getReadChannels().get(0),
                                    inputInstance.getLocalMemoryOfHwExecutionNode(hwNode));

                        if (!taskCast.getWriteChannels().isEmpty())
                            tmlMapping.addCommToHwCommNode(taskCast.getWriteChannels().get(0),
                                    inputInstance.getLocalMemoryOfHwExecutionNode(hwNode));
                    ...
                    optimizedSolutionX.put("X[" + taskCast.getName() + "][" + hwNode.getName() + "] = ",
                            Integer.parseInt(optimized_result_X[t][p].toString()));

                    // TraceManager.addDev("X[" + task.getName() + "][" + hwNode.getName() + "] = "
            ...
            for (Object task : inputInstance.getModeling().getTasks()) {
                ...
                // optimizedSolutionStart.put("start[" + taskCast.getName() + "] ",
                // Integer.parseInt(optimized_result_start[t].toString()));

                // TraceManager.addDev("start[" + task.getName() + "] = " +
                // optimized_result_start[t]);
