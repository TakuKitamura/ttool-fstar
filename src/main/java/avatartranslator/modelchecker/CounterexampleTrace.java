...
public class CounterexampleTrace {
    ...
    public boolean buildTrace(Map<Integer, SpecificationState> states,
            Map<Integer, CounterexampleTraceState> counterstates) {
        if (counterexampleState == null) {
            return false;
        }
    ...
    public String generateSimpleTrace() {
        ...
        for (CounterexampleTraceState cs : trace) {
            ...
            int j = 0;
            for (AvatarBlock block : blocks) {
                s.append("\t" + block.getName() + "."
                        + block.getStateMachine().allStates[cs.blocksStates[j++]].getName());
            }
            s.append("\n");
            id++;
        }
    ...
    public void generateTraceAUT(String query, Map<Integer, SpecificationState> states) {
        ...
        for (CounterexampleTraceState cs : trace) {
            ...
            if (state != null && state.nexts != null) {
                for (SpecificationLink sl : state.nexts) {
                    if (sl.destinationState.hashValue == cs.hash) {
                        s.append("(" + statesID.get(sl.originState.id) + ",\"" + sl.action + "\","
                                + statesID.get(sl.destinationState.id) + ")\n");
                        break;
                    }
                }
            }
    ...
    private boolean findLoopTraceRec(SpecificationState start, SpecificationState arrival, Set<Long> visited,
            List<SpecificationState> loopTrace, int depth) {
