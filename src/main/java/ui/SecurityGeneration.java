...
public class SecurityGeneration implements Runnable {
    ...
    public SecurityGeneration(MainGUI gui, String name, TMLMapping<TGComponent> map, TMLArchiPanel newarch,
            String encComp, String overhead, String decComp, boolean autoConf, boolean autoWeakAuth,
        ...
        try {
            ...
            for (AvatarPragmaSecret pragma : confResults.keySet()) {
                ...
                if (confResults.get(pragma).isProved() && !confResults.get(pragma).isSatisfied()) {
                    ...
                    TraceManager.addDev("SECGEN:" + pragma.getArg().getBlock().getName() + "."
                            + pragma.getArg().getName() + " is not secret");

                    TMLChannel chan = map.getTMLModeling()
                            .getChannelByShortName(pragma.getArg().getName().replaceAll("_chData", ""));

                    if (chan == null) {
                        chan = map.getTMLModeling()
                                .getChannelByOriginPortName(pragma.getArg().getName().replaceAll("_chData", ""));
                    }
            ...
            for (AvatarPragmaAuthenticity pragma : authResults.keySet()) {
                ...
                if (authResults.get(pragma).isProved() && !authResults.get(pragma).isSatisfied()) {
                    nonAuthChans.add(pragma.getAttrA().getAttribute().getBlock().getName() + "__"
                            + pragma.getAttrA().getAttribute().getName().replaceAll("_chData", ""));
                    nonAuthChans.add(pragma.getAttrB().getAttribute().getBlock().getName() + "__"
                            + pragma.getAttrB().getAttribute().getName().replaceAll("_chData", ""));
                }
            }
    ...
    public boolean portInTask(TMLTask task, String portName) {
        TMLActivityDiagramPanel tad = tmlcdp.getTMLActivityDiagramPanel(task.getName());// FIXME (
                                                                                        // getActivityDiagramName(
                                                                                        // task ) );
    ...
    public void run() {
        ...
        // ToSecure keeps a map of origin task: {dest task} for which security operators
        // need to be added
        // ToSecureRev keeps a map of dest task: {origin task} for which security
        ...
        // nonceInChannels are channels which need to send a nonce before receiving the
        // channel data
        // nonceOutChannels are channels which need to receive a nonce before sending
        // the channel data
        ...
        // hsmSecInChannels need to send data to the hsm to decrypt after receiving
        // channel data
        // hsmSecOutChannels need to send data to the hsm to encrypt before sending
        // channel data
        ...
        for (TMLTask task : map.getTMLModeling().getTasks()) {
            ...
            for (TMLChannel chan : tmlmodel.getChannels(task)) {
                // Origin and Destination ports can have different names. Therefore, we need to
                // put both names in the list of channels to secure.
                ...
                if (chan.isBasicChannel()) {
                    ...
                    if (nonConfChans.contains(chan.getOriginTask().getName().split("__")[1] + "__"
                            + chan.getOriginPort().getName() + "_chData")) {
                        nonConf = true;
                        TraceManager.addDev("SECGEN. non conf basic ch = true");
                    }

                    if (nonAuthChans.contains(chan.getDestinationTask().getName().split("__")[1] + "__" + title + "__"
                            + chan.getDestinationPort().getName())) {
                        nonAuth = true;
                    }

                    // When port names are different
                    if (nonAuthChans
                            .contains(chan.getDestinationTask().getName().split("__")[1] + "__" + chan.getName())) {
                        nonAuth = true;
                    }
                } else {
                    ...
                    for (TMLTask destTask : chan.getDestinationTasks()) {
                        for (TMLPort port : chan.getDestinationPorts()) {
                            if (nonAuthChans.contains(
                                    destTask.getName().split("__")[1] + "__" + title + "__" + port.getName())) {
                                nonAuth = true;
                            }
                        ...
                        // System.out.println(destTask.getName().split("__")[1] + "__" +
                        // chan.getName());
                ...
                for (String chanName : portNames) {
                    // Classify channels based on the type of security requirements and unsatisfied
                    // properties
                    if (chan.isBasicChannel()) {
                        if (chan.checkConf && autoConf && nonConf) {
                            ...
                            if (hsmTasks.contains(chan.getOriginTask().getName().split("__")[1])) {
                                ...
                                if (!hsmSecOutChannels.get(chan.getOriginTask()).contains(chanName)
                                        && portInTask(chan.getOriginTask(), chanName)) {
                                    HSMChannel hsmchan = new HSMChannel(chanName,
                                            chan.getOriginTask().getName().split("__")[1], HSMChannel.SENC);
                                    hsmChannelMap.get(taskHSMMap.get(chan.getOriginTask().getName().split("__")[1]))
                                            .add(hsmchan);
                                    hsmSecOutChannels.get(chan.getOriginTask()).add(chanName);

                                    if (chan.checkAuth && autoStrongAuth) {
                                        nonceOutChannels.get(chan.getOriginTask()).add(chanName);
                                        hsmchan.nonceName = "nonce_"
                                                + chan.getDestinationTask().getName().split("__")[1] + "_"
                                                + chan.getOriginTask().getName().split("__")[1];
                                    }
                                }
                            } else {
                            ...
                            if (hsmTasks.contains(chan.getDestinationTask().getName().split("__")[1])) {
                                if (!hsmSecInChannels.get(chan.getDestinationTask()).contains(chanName)
                                        && portInTask(chan.getDestinationTask(), chanName)) {
                                    HSMChannel hsmchan = new HSMChannel(chanName,
                                            chan.getDestinationTask().getName().split("__")[1], HSMChannel.DEC);
                                    hsmChannelMap
                                            .get(taskHSMMap.get(chan.getDestinationTask().getName().split("__")[1]))
                                            .add(hsmchan);
                            ...
                            if (autoStrongAuth) {
                                ...
                                /*
                                 * } else { TMLChannel chantmp =
                                 * oldmodel.getChannelByShortName("__"+chan.getName());
                            ...
                            if (hsmTasks.contains(chan.getOriginTask().getName().split("__")[1])) {
                                ...
                                if (!hsmSecOutChannels.get(chan.getOriginTask()).contains(chanName)
                                        && portInTask(chan.getOriginTask(), chanName)) {
                                    HSMChannel hsmchan = new HSMChannel(chanName,
                                            chan.getOriginTask().getName().split("__")[1], HSMChannel.MAC);
                                    hsmChannelMap.get(taskHSMMap.get(chan.getOriginTask().getName().split("__")[1]))
                                            .add(hsmchan);
                                    hsmSecOutChannels.get(chan.getOriginTask()).add(chanName);
                        ...
                        for (TMLTask orig : chan.getOriginTasks()) {
                            for (TMLTask dest : chan.getDestinationTasks()) {
                                if (chan.checkConf && autoConf && nonConf) {
                                    ...
                                    /*
                                     * if (chan.checkAuth && autoStrongAuth) { if
                                     * (!toSecureRev.get(dest).contains(orig)){ toSecureRev.get(dest).add(orig); } }
                                     */
                                    if (hsmTasks.contains(orig.getName().split("__")[1])) {
                                        channelSecMap.put(chanName, "hsmSec_" + secName);
                                        if (!hsmSecOutChannels.get(orig).contains(chanName)
                                                && portInTask(orig, chanName)) {
                                            HSMChannel hsmchan = new HSMChannel(chanName, orig.getName().split("__")[1],
                                                    HSMChannel.SENC);
                                            hsmChannelMap.get(taskHSMMap.get(orig.getName().split("__")[1]))
                                                    .add(hsmchan);
                                            hsmSecOutChannels.get(orig).add(chanName);

                                            /*
                                             * if (chan.checkAuth && autoStrongAuth) {
                                             * nonceOutChannels.get(orig).add(chanName); hsmchan.nonceName="nonce_" +
                                             * dest.getName().split("__")[1] + "_" + orig.getName().split("__")[1]; }
                                             */
                                        }
                                    } else {
                                    ...
                                    if (hsmTasks.contains(dest.getName().split("__")[1])) {
                                        if (!hsmSecInChannels.get(dest).contains(chanName)
                                                && portInTask(dest, chanName)) {
                                            HSMChannel hsmchan = new HSMChannel(chanName, dest.getName().split("__")[1],
                                                    HSMChannel.DEC);
                                            hsmChannelMap.get(taskHSMMap.get(dest.getName().split("__")[1]))
                                                    .add(hsmchan);
                                            hsmSecInChannels.get(dest).add(chanName);
                                            /*
                                             * if (chan.checkAuth && autoStrongAuth) {
                                             * nonceInChannels.get(dest).add(chanName); hsmchan.nonceName="nonce_" +
                                             * dest.getName().split("__")[1] + "_" + orig.getName().split("__")[1]; }
                                             */
                                        }
                                    } else {
                                    ...
                                    toSecure.get(orig).add(dest);
                                    /*
                                     * if (autoStrongAuth) { /* if (chan.getOriginTask().getReferenceObject()
                                     * instanceof TMLCPrimitiveComponent &&
                                     * chan.getDestinationTask().getReferenceObject() instanceof
                                     * TMLCPrimitiveComponent){
                                     */
                                    /*
                                     * if (!toSecureRev.get(dest).contains(orig)){ toSecureRev.get(dest).add(orig);
                                     * }
                                     */
                                    /*
                                     * } else { TMLChannel chantmp =
                                    ...
                                    if (hsmTasks.contains(orig.getName().split("__")[1])) {
                                        ...
                                        if (!hsmSecOutChannels.get(orig).contains(chanName)
                                            ...
                                            /*
                                             * if (autoStrongAuth) { nonceOutChannels.get(orig).add(chanName);
                                             * hsmchan.nonceName="nonce_" + dest.getName().split("__")[1] + "_" +
                                             * orig.getName().split("__")[1]; }
                                             */
                                        }
                                    } else {
        ...
        // Add a HSM Task for each selected CPU on the component diagram, add associated
        // channels, etc
        for (String cpuName : selectedCPUTasks.keySet()) {
            TMLCPrimitiveComponent hsm = new TMLCPrimitiveComponent(0, tcdp.getMaxY() - 200, tcdp.getMinX(),
                    tcdp.getMaxX(), tcdp.getMinY(), tcdp.getMaxY(), false, null, tcdp);
            ...
            for (TMLCPrimitiveComponent comp : comps) {
                ...
                for (ChannelData hsmChan : hsmChans) {
                    TMLCChannelOutPort originPort = new TMLCChannelOutPort(comp.getX(), comp.getY(), tcdp.getMinX(),
                            tcdp.getMaxX(), tcdp.getMinY(), tcdp.getMaxX(), true, hsm, tcdp);
                    TMLCChannelOutPort destPort = new TMLCChannelOutPort(comp.getX(), comp.getY(), tcdp.getMinX(),
                            tcdp.getMaxX(), tcdp.getMinY(), tcdp.getMaxX(), true, comp, tcdp);
                    ...
                    TMLCPortConnector conn = new TMLCPortConnector(0, 0, tcdp.getMinX(), tcdp.getMaxX(), tcdp.getMinY(),
                            tcdp.getMaxX(), true, null, tcdp, originPort.getTGConnectingPointAtIndex(0),
        ...
        for (String cpuName : selectedCPUTasks.keySet()) {
            ...
            // Add new memory
            TMLArchiMemoryNode mem = new TMLArchiMemoryNode(cpu.getX(), archPanel.getMaxY() - 800, archPanel.getMinX(),
                    archPanel.getMaxX(), archPanel.getMinY(), archPanel.getMaxY(), true, null, archPanel);
            ...
            TMLArchiHWANode hwa = new TMLArchiHWANode(cpu.getX(), archPanel.getMaxY() - 300, archPanel.getMinX(),
                    archPanel.getMaxX(), archPanel.getMinY(), archPanel.getMaxY(), true, null, archPanel);
            archPanel.addComponent(hwa, cpu.getX() + 100, cpu.getY() + 100, false, true);
            hwa.setName("HSM_" + cpuName);
            // Add hsm task to hwa

            TMLArchiArtifact hsmArt = new TMLArchiArtifact(cpu.getX(), archPanel.getMaxY() - 280, archPanel.getMinX(),
                    archPanel.getMaxX(), archPanel.getMinY(), archPanel.getMaxY(), true, hwa, archPanel);
            ...
            TMLArchiBUSNode bus = new TMLArchiBUSNode(cpu.getX() + 50, archPanel.getMaxY() - 500, archPanel.getMinX(),
                    archPanel.getMaxX(), archPanel.getMinY(), archPanel.getMaxY(), true, null, archPanel);
            bus.setPrivacy(1);
            ...
            // Connect Bus and CPU
            TMLArchiConnectorNode connect = new TMLArchiConnectorNode(cpu.getX() + 100, archPanel.getMaxY() - 300,
                    archPanel.getMinX(), archPanel.getMaxX(), archPanel.getMinY(), archPanel.getMaxY(), true, null,
                    archPanel, null, null, new Vector<Point>());
            ...
            connect = new TMLArchiConnectorNode(cpu.getX() + 100, cpu.getY() + 100, archPanel.getMinX(),
                    archPanel.getMaxX(), archPanel.getMinY(), archPanel.getMaxY(), true, null, archPanel, null, null,
                    new Vector<Point>());
        ...
        // int num=0;
        // int nonceNum=0;
        // Create reverse channels on component diagram to send nonces if they don't
        // already exist
        ...
        for (TMLTask task : toSecureRev.keySet()) {
            ...
            for (TMLTask task2 : toSecureRev.get(task)) {
                ...
                if (addChan) {
                    TMLCChannelOutPort originPort = new TMLCChannelOutPort(0, 0, tcdp.getMinX(), tcdp.getMaxX(),
                            tcdp.getMinY(), tcdp.getMaxX(), true, null, tcdp);
                    TMLCChannelOutPort destPort = new TMLCChannelOutPort(0, 0, tcdp.getMinX(), tcdp.getMaxX(),
                            tcdp.getMinY(), tcdp.getMaxX(), true, null, tcdp);

                    if (task.getReferenceObject() instanceof TMLCPrimitiveComponent) {
                        TGComponent tg = (TGComponent) task.getReferenceObject();
                        originPort = new TMLCChannelOutPort(tg.getX(), tg.getY(), tcdp.getMinX(), tcdp.getMaxX(),
                                tcdp.getMinY(), tcdp.getMaxX(), true, tg, tcdp);
                        originPort.commName = "nonceCh" + task.getName().split("__")[1] + "_"
                                + task2.getName().split("__")[1];
                    ...
                    if (task2.getReferenceObject() instanceof TMLCPrimitiveComponent) {
                        ...
                        destPort = new TMLCChannelOutPort(tg.getX(), tg.getY(), tcdp.getMinX(), tcdp.getMaxX(),
                                tcdp.getMinY(), tcdp.getMaxX(), true, tg, tcdp);
                        destPort.isOrigin = false;
                        destPort.commName = "nonceCh" + task.getName().split("__")[1] + "_"
                                + task2.getName().split("__")[1];
                        tcdp.addComponent(destPort, tg.getX(), tg.getY(), true, true);
                    }
                    tmlmodel.addChannel(new TMLChannel(
                            "nonceCh" + task.getName().split("__")[1] + "_" + task2.getName().split("__")[1],
                            originPort));
                    // Add connection
                    TMLCPortConnector conn = new TMLCPortConnector(0, 0, tcdp.getMinX(), tcdp.getMaxX(), tcdp.getMinY(),
                            tcdp.getMaxX(), true, null, tcdp, originPort.getTGConnectingPointAtIndex(0),
        ...
        for (TMLTask task : toSecure.keySet()) {
            ...
            int xpos = 0;
            int ypos = 0;
            TGConnector fromStart = new TGConnectorTMLAD(0, 0, 0, 0, 0, 0, false, null, tad, null, null,
                    new Vector<Point>());
            ...
            // For each occurence of a write channel operator, add encryption/nonces before
            // it
            ...
            for (String channel : secOutChannels.get(task)) {
                ...
                // First, find the connector that points to it. We will add the encryption,
                // nonce operators directly before the write channel operator
                for (TGComponent tg : tad.getComponentList()) {
                    if (tg instanceof TMLADWriteChannel) {
                        TMLADWriteChannel writeChannel = (TMLADWriteChannel) tg;
                        if (writeChannel.getChannelName().equals(channel)
                                && writeChannel.getSecurityContext().equals("")) {
                ...
                for (TGComponent comp : channelInstances) {
                    ...
                    TMLADEncrypt enc = new TMLADEncrypt(xpos, ypos + yShift, tad.getMinX(), tad.getMaxX(),
                    ...
                    if (nonceOutChannels.get(task).contains(channel)) {
                        ...
                        rd = new TMLADReadChannel(xpos, ypos + yShift, tad.getMinX(), tad.getMaxX(), tad.getMinY(),
                                tad.getMaxY(), false, null, tad);
                        // System.out.println("tmlc " + tmlc);
                        // System.out.println("Checking "+ tmlc.getDestinationTask() + " " +
                        // tmlc.getOriginTask());
                        List<TMLChannel> matches = tmlmodel.getChannels(tmlc.getDestinationTask(),
                                tmlc.getOriginTask());

                        if (matches.size() > 0) {
                            rd.setChannelName(matches.get(0).getName().replaceAll(title + "__", ""));
                        } else {
                            rd.setChannelName("nonceCh" + tmlc.getDestinationTask().getName().split("__")[1] + "_"
                                    + tmlc.getOriginTask().getName().split("__")[1]);
                        }
                        rd.setSecurityContext("nonce_" + tmlc.getDestinationTask().getName().split("__")[1] + "_"
                                + tmlc.getOriginTask().getName().split("__")[1]);
                        tad.addComponent(rd, xpos, ypos + yShift, false, true);
                        fromStart.setP2(rd.getTGConnectingPointAtIndex(0));
                        fromStart = new TGConnectorTMLAD(enc.getX(), enc.getY(), tad.getMinX(), tad.getMaxX(),
                                tad.getMinY(), tad.getMaxY(), false, null, tad, null, null, new Vector<Point>());
                        ...
                        if (tmlc != null) {
                            enc.nonce = "nonce_" + tmlc.getDestinationTask().getName().split("__")[1] + "_"
                                    + tmlc.getOriginTask().getName().split("__")[1];
                        }
                    }
                    ...
                    // Shift components down to make room for the added ones, and add security
                    // contexts to write channels
                    for (TGComponent tg : tad.getComponentList()) {
                        if (tg instanceof TMLADWriteChannel) {
            ...
            for (String channel : hsmSecOutChannels.get(task)) {
                Set<TGComponent> channelInstances = new HashSet<TGComponent>();
                // TGConnector conn = new TGConnectorTMLAD(0, 0, 0, 0, 0, 0, false, null, tad,
                // null, null, new Vector<Point>());
                ...
                for (TGComponent chan : channelInstances) {
                    ...
                    TMLADSendRequest req = new TMLADSendRequest(xpos, ypos + yShift, tad.getMinX(), tad.getMaxX(),
                            tad.getMinY(), tad.getMaxY(), false, null, tad);
                    ...
                    // Add connection
                    fromStart = new TGConnectorTMLAD(xpos, ypos, tad.getMinX(), tad.getMaxX(), tad.getMinY(),
                            tad.getMaxY(), false, null, tad, null, null, new Vector<Point>());
                    fromStart.setP1(req.getTGConnectingPointAtIndex(1));

                    TMLADWriteChannel wr = new TMLADWriteChannel(xpos, ypos + yShift, tad.getMinX(), tad.getMaxX(),
                            tad.getMinY(), tad.getMaxY(), false, null, tad);

                    yShift += 50;
                    // Add write channel operator
                    wr = new TMLADWriteChannel(xpos, ypos + yShift, tad.getMinX(), tad.getMaxX(), tad.getMinY(),
                            tad.getMaxY(), false, null, tad);
                    ...
                    TMLADReadChannel rd = new TMLADReadChannel(xpos, ypos + yShift, tad.getMinX(), tad.getMaxX(),
                            tad.getMinY(), tad.getMaxY(), false, null, tad);

                    TMLADWriteChannel wr2 = new TMLADWriteChannel(xpos, ypos + yShift, tad.getMinX(), tad.getMaxX(),
                            tad.getMinY(), tad.getMaxY(), false, null, tad);
                    ...
                    if (nonceOutChannels.get(task).contains(channel)) {
                        ...
                        fromStart = new TGConnectorTMLAD(rd.getX(), rd.getY(), tad.getMinX(), tad.getMaxX(),
                                tad.getMinY(), tad.getMaxY(), false, null, tad, null, null, new Vector<Point>());
                        tad.addComponent(fromStart, xpos, ypos, false, true);
                        fromStart.setP1(rd.getTGConnectingPointAtIndex(1));
                        ...
                        yShift += 50;
                        wr2 = new TMLADWriteChannel(xpos, ypos + yShift, tad.getMinX(), tad.getMaxX(), tad.getMinY(),
                                tad.getMaxY(), false, null, tad);
                        ...
                        TGConnectorTMLAD tmp = new TGConnectorTMLAD(wr2.getX(), wr2.getY() + yShift, tad.getMinX(),
                                tad.getMaxX(), tad.getMinY(), tad.getMaxY(), false, null, tad,
                                rd.getTGConnectingPointAtIndex(1), wr2.getTGConnectingPointAtIndex(0),
                                new Vector<Point>());
                    ...
                    yShift += 60;
                    TMLADReadChannel rd2 = new TMLADReadChannel(xpos, ypos + yShift, tad.getMinX(), tad.getMaxX(),
                            tad.getMinY(), tad.getMaxY(), false, null, tad);
            ...
            for (String channel : hsmSecInChannels.get(task)) {
                // System.out.println("Checking hsmsecinchannel " + channel + " " +
                // task.getName());
                Set<TGComponent> channelInstances = new HashSet<TGComponent>();
                // TGConnector conn = new TGConnectorTMLAD(0, 0, 0, 0, 0, 0, false, null, tad,
                // null, null, new Vector<Point>());
                ...
                for (TGComponent tg : tad.getComponentList()) {
                    if (tg instanceof TMLADReadChannel) {
                        TMLADReadChannel readChannel = (TMLADReadChannel) tg;
                        if (readChannel.getChannelName().equals(channel)
                                && readChannel.getSecurityContext().equals("")) {
                            fromStart = tad.findTGConnectorEndingAt(tg.getTGConnectingPointAtIndex(0));
                            if (fromStart != null) {
                                channelInstances.add(tg);
                            }
                ...
                for (TGComponent chan : channelInstances) {
                    ...
                    TMLADEncrypt nonce = new TMLADEncrypt(xpos, ypos + yShift, tad.getMinX(), tad.getMaxX(),
                            tad.getMinY(), tad.getMaxY(), false, null, tad);
                    TMLADWriteChannel wr3 = new TMLADWriteChannel(xpos, ypos + yShift, tad.getMinX(), tad.getMaxX(),
                            tad.getMinY(), tad.getMaxY(), false, null, tad);
                    TMLADWriteChannel wr2 = new TMLADWriteChannel(xpos, ypos + yShift, tad.getMinX(), tad.getMaxX(),
                            tad.getMinY(), tad.getMaxY(), false, null, tad);
                    ...
                    if (nonceInChannels.get(task).contains(channel)) {
                        ...
                        yShift += 60;
                        nonce = new TMLADEncrypt(xpos, ypos + yShift, tad.getMinX(), tad.getMaxX(), tad.getMinY(),
                                tad.getMaxY(), false, null, tad);
                        nonce.securityContext = "nonce_" + tmlc.getDestinationTask().getName().split("__")[1] + "_"
                                + tmlc.getOriginTask().getName().split("__")[1];
                        nonce.type = "Nonce";
                        ...
                        wr3 = new TMLADWriteChannel(xpos, ypos + yShift, tad.getMinX(), tad.getMaxX(), tad.getMinY(),
                                tad.getMaxY(), false, null, tad);
                        // Send nonce along channel, the newly created nonce channel or an existing
                        // channel with the matching sender and receiver
                        ...
                        if (matches.size() > 0) {
                            ...
                            wr3.setChannelName("nonceCh" + tmlc.getDestinationTask().getName().split("__")[1] + "_"
                                    + tmlc.getOriginTask().getName().split("__")[1]);
                        }
                        // send the nonce along the channel
                        wr3.setSecurityContext("nonce_" + tmlc.getDestinationTask().getName().split("__")[1] + "_"
                                + tmlc.getOriginTask().getName().split("__")[1]);
                        tad.addComponent(wr3, xpos, ypos + yShift, false, true);

                        wr3.makeValue();
                        TGConnector tmp = new TGConnectorTMLAD(wr3.getX(), wr3.getY() + yShift, tad.getMinX(),
                                tad.getMaxX(), tad.getMinY(), tad.getMaxY(), false, null, tad,
                                nonce.getTGConnectingPointAtIndex(1), wr3.getTGConnectingPointAtIndex(0),
                                new Vector<Point>());
                        tad.addComponent(tmp, xpos, ypos, false, true);
                        ...
                        wr2 = new TMLADWriteChannel(xpos, ypos + yShift, tad.getMinX(), tad.getMaxX(), tad.getMinY(),
                                tad.getMaxY(), false, null, tad);
                        wr2.setChannelName("data_" + chanName + "_" + task.getName().split("__")[1]);
                        wr2.setSecurityContext("nonce_" + tmlc.getDestinationTask().getName().split("__")[1] + "_"
                                + tmlc.getOriginTask().getName().split("__")[1]);
                        tad.addComponent(wr2, xpos, ypos + yShift, false, true);

                        tmp = new TGConnectorTMLAD(wr2.getX(), wr2.getY() + yShift, tad.getMinX(), tad.getMaxX(),
                                tad.getMinY(), tad.getMaxY(), false, null, tad, wr3.getTGConnectingPointAtIndex(1),
                                wr2.getTGConnectingPointAtIndex(0), new Vector<Point>());
                        tad.addComponent(tmp, xpos, ypos, false, true);

                        fromStart = new TGConnectorTMLAD(wr2.getX(), wr.getY(), tad.getMinX(), tad.getMaxX(),
                                tad.getMinY(), tad.getMaxY(), false, null, tad, wr2.getTGConnectingPointAtIndex(1),
                                null, new Vector<Point>());
                        tad.addComponent(fromStart, xpos, ypos, false, true);
                        ...
                        /*
                         * //Shift everything from the read channel on down for (TGComponent tg :
                         * tad.getComponentList()) { if (tg.getY() >= ypos && tg != nonce && tg != wr2
                         * && tg!=wr3) { tg.setCd(tg.getX(), tg.getY() + yShift); } }
                         */
                    }
                    ...
                    // Direct the last TGConnector back to the start of the operator after the read
                    // channel
            ...
            for (String channel : macInChannels.get(task)) {
                ...
                int yShift = 50;
                Set<TGComponent> channelInstances = new HashSet<TGComponent>();
                TGConnector conn = new TGConnectorTMLAD(0, 0, 0, 0, 0, 0, false, null, tad, null, null,
                        new Vector<Point>());
                TGConnectingPoint next = new TGConnectingPoint(null, 0, 0, false, false);
                ...
                for (TGComponent comp : channelInstances) {
                    ...
                    TMLADWriteChannel wr = new TMLADWriteChannel(0, 0, tad.getMinX(), tad.getMaxX(), tad.getMinY(),
                            tad.getMaxY(), false, null, tad);
                    ...
                    if (nonceInChannels.get(task).contains(channel)) {
                        ...
                        if (matches.size() > 0) {
                            ...
                            wr.setChannelName("nonceCh" + tmlc.getDestinationTask().getName().split("__")[1] + "_"
                                    + tmlc.getOriginTask().getName().split("__")[1]);
                        }
                        // send the nonce along the channel
                        wr.setSecurityContext("nonce_" + tmlc.getDestinationTask().getName().split("__")[1] + "_"
                                + tmlc.getOriginTask().getName().split("__")[1]);
                        tad.addComponent(wr, xpos, ypos + yShift, false, true);
                        wr.makeValue();
                        TGConnector tmp = new TGConnectorTMLAD(wr.getX(), wr.getY() + yShift, tad.getMinX(),
                                tad.getMaxX(), tad.getMinY(), tad.getMaxY(), false, null, tad,
                                nonce.getTGConnectingPointAtIndex(1), wr.getTGConnectingPointAtIndex(0),
                                new Vector<Point>());
                        tad.addComponent(tmp, xpos, ypos, false, true);
                        fromStart = new TGConnectorTMLAD(wr.getX(), wr.getY(), tad.getMinX(), tad.getMaxX(),
                                tad.getMinY(), tad.getMaxY(), false, null, tad, wr.getTGConnectingPointAtIndex(1), null,
                                new Vector<Point>());
                        tad.addComponent(fromStart, xpos, ypos, false, true);
                        // Connect created write channel operator to start of read channel operator
                        fromStart.setP1(wr.getTGConnectingPointAtIndex(1));
                    ...
                    TMLADDecrypt dec = new TMLADDecrypt(xpos + 10, ypos + yShift, tad.getMinX(), tad.getMaxX(),
                            tad.getMinY(), tad.getMaxY(), false, null, tad);
                    ...
                    yShift += 60;
                    conn = new TGConnectorTMLAD(xpos, ypos + yShift, tad.getMinX(), tad.getMaxX(), tad.getMinY(),
                            tad.getMaxY(), false, null, tad, dec.getTGConnectingPointAtIndex(1), next,
                            new Vector<Point>());
                    ...
                    for (TGComponent tg : tad.getComponentList()) {
                        if (tg instanceof TMLADReadChannel) {
                            ...
                            if (channel.equals(readChannel.getChannelName())
                                    && readChannel.getSecurityContext().equals("")) {
                                readChannel.setSecurityContext(channelSecMap.get(readChannel.getChannelName()));
                                readChannel.setEncForm(true);
            ...
            for (String channel : secInChannels.get(task)) {
                ...
                TMLADReadChannel readChannel = new TMLADReadChannel(xpos, ypos + yShift, tad.getMinX(), tad.getMaxX(),
                        tad.getMinY(), tad.getMaxY(), false, null, tad);
                ...
                for (TGComponent comp : channelInstances) {
                    ...
                    TMLADWriteChannel wr = new TMLADWriteChannel(0, 0, tad.getMinX(), tad.getMaxX(), tad.getMinY(),
                            tad.getMaxY(), false, null, tad);
                    TMLADEncrypt nonce = new TMLADEncrypt(0, 0, tad.getMinX(), tad.getMaxX(), tad.getMinY(),
                            tad.getMaxY(), false, null, tad);
                    if (nonceInChannels.get(task).contains(channel)) {
                        // Create a nonce operator and a write channel operator
                        nonce = new TMLADEncrypt(xpos, ypos + yShift, tad.getMinX(), tad.getMaxX(), tad.getMinY(),
        ...
        // }
        /*
         * for (TMLTask task : newmodel.getTasks()) { task.setName(tabName + "_" + name
         * + "__" + task.getName()); } for (TMLTask task : tmlmodel.getTasks()) {
         * HwExecutionNode node = (HwExecutionNode) map.getHwNodeOf(task); if
         * (newmodel.getTMLTaskByName(task.getName().replace(tabName, tabName + "_" +
         * name)) != null) {
         * map.addTaskToHwExecutionNode(newmodel.getTMLTaskByName(task.getName().replace
         * (tabName, tabName + "_" + name)), node); map.removeTask(task); } else {
         * 
         * } }
         */
    ...
    public void buildHSMActivityDiagram(String cpuName) {
        ...
        fromStart = new TGConnectorTMLAD(xpos, ypos, tad.getMinX(), tad.getMaxX(), tad.getMinY(), tad.getMaxY(), false,
                null, tad, null, null, new Vector<Point>());

        if (hsmChannelMap.get(cpuName).size() == 0) {
            TMLADStopState stop = new TMLADStopState(100, 100, tad.getMinX(), tad.getMaxX(), tad.getMinY(),
                    tad.getMaxY(), false, null, tad);
            tad.addComponent(stop, 100, 100, false, true);

            // Connect stop and start
            fromStart = new TGConnectorTMLAD(xpos, ypos, tad.getMinX(), tad.getMaxX(), tad.getMinY(), tad.getMaxY(),
        ...
        TMLADReadRequestArg req = new TMLADReadRequestArg(300, 100, tad.getMinX(), tad.getMaxX(), tad.getMinY(),
                tad.getMaxY(), false, null, tad);
        ...
        TMLADChoice choice = new TMLADChoice(300, 200, tad.getMinX(), tad.getMaxX(), tad.getMinY(), tad.getMaxY(),
                false, null, tad);
        ...
        if (hsmChannelMap.get(cpuName).size() > 3) {
            TMLADChoice choice2 = new TMLADChoice(xc, 400, tad.getMinX(), tad.getMaxX(), tad.getMinY(), tad.getMaxY(),
                    false, null, tad);
            int i = 0;
            for (HSMChannel ch : hsmChannelMap.get(cpuName)) {
                ...
                if (i % 3 == 0) {
                    ...
                    choice2 = new TMLADChoice(xc, 250, tad.getMinX(), tad.getMaxX(), tad.getMinY(), tad.getMaxY(),
                            false, null, tad);
                ...
                TMLADReadChannel rd = new TMLADReadChannel(xc, 300, tad.getMinX(), tad.getMaxX(), tad.getMinY(),
                        tad.getMaxY(), false, null, tad);
                ...
                tad.addComponent(fromStart, 300, 200, false, true);
                TMLADWriteChannel wr = new TMLADWriteChannel(xc, 600, tad.getMinX(), tad.getMaxX(), tad.getMinY(),
                        tad.getMaxY(), false, null, tad);
                ...
                if (ch.secType == HSMChannel.DEC) {
                    TMLADDecrypt dec = new TMLADDecrypt(xc, 500, tad.getMinX(), tad.getMaxX(), tad.getMinY(),
                            tad.getMaxY(), false, null, tad);
                    ...
                    TMLADEncrypt enc = new TMLADEncrypt(xc, 500, tad.getMinX(), tad.getMaxX(), tad.getMinY(),
                            tad.getMaxY(), false, null, tad);
                    ...
                    // Add Stop
                    TMLADStopState stop = new TMLADStopState(xc, 600, tad.getMinX(), tad.getMaxX(), tad.getMinY(),
                            tad.getMaxY(), false, null, tad);
            ...
            for (HSMChannel ch : hsmChannelMap.get(cpuName)) {
                ...
                fromStart = new TGConnectorTMLAD(xc, 350, tad.getMinX(), tad.getMaxX(), tad.getMinY(), tad.getMaxY(),
                        false, null, tad, null, null, new Vector<Point>());
                fromStart.setP1(rd.getTGConnectingPointAtIndex(1));
                ...
                if (!ch.nonceName.equals("")) {
                    ...
                    rd = new TMLADReadChannel(xc, 350, tad.getMinX(), tad.getMaxX(), tad.getMinY(), tad.getMaxY(),
                            false, null, tad);
                    rd.setChannelName("data_" + ch.name + "_" + ch.task);
                    ...
                    fromStart = new TGConnectorTMLAD(xc, 350, tad.getMinX(), tad.getMaxX(), tad.getMinY(),
                            tad.getMaxY(), false, null, tad, null, null, new Vector<Point>());
                    fromStart.setP1(rd.getTGConnectingPointAtIndex(1));
                ...
                if (ch.secType == HSMChannel.DEC) {
                    ...
                    // Add Stop
                    TMLADStopState stop = new TMLADStopState(xc, 700, tad.getMinX(), tad.getMaxX(), tad.getMinY(),
                            tad.getMaxY(), false, null, tad);
                    tad.addComponent(stop, xc, 700, false, true);
