...
public class HSMGeneration implements Runnable {
    ...
    try {
      ...
      for (AvatarPragmaSecret pragma : confResults.keySet()) {
        if (confResults.get(pragma).isProved() && !confResults.get(pragma).isSatisfied()) {
          ...
          TraceManager
              .addDev(pragma.getArg().getBlock().getName() + "." + pragma.getArg().getName() + " is not secret");
  ...
  public void run() {
    ...
    for (String cpuName : selectedCpuTasks.keySet()) {
      ...
      TMLCPrimitiveComponent hsm = new TMLCPrimitiveComponent(0, 500, tcdp.getMinX(), tcdp.getMaxX(), tcdp.getMinY(),
          tcdp.getMaxY(), false, null, tcdp);
    ...
    for (TMLTask task : tmap.getTMLModeling().getTasks()) {
      ...
      for (String chan : secChannels.keySet()) {
        ...
        for (TGComponent chI : channelInstances) {
          ...
          int yShift = 100;
          TMLADDecrypt dec = new TMLADDecrypt(xpos, ypos + yShift, tad.getMinX(), tad.getMaxX(), tad.getMinY(),
              tad.getMaxY(), false, null, tad);
          dec.securityContext = ch.securityContext;
          ...
          fromStart.setP2(dec.getTGConnectingPointAtIndex(0));
          if (point != null) {
            fromStart = new TGConnectorTMLAD(dec.getX(), dec.getY(), tad.getMinX(), tad.getMaxX(), tad.getMinY(),
      ...
      for (String chan : secChannels.keySet()) {
        ...
        for (TGComponent chI : channelInstances) {
          ...
          TMLADEncrypt enc = new TMLADEncrypt(xpos, ypos, tad.getMinX(), tad.getMaxX(), tad.getMinY(), tad.getMaxY(),
              false, null, tad);
    ...
    for (String cpuName : selectedCpuTasks.keySet()) {
      ...
      // Connect Bus and CPU
      TMLArchiConnectorNode connect = new TMLArchiConnectorNode(cpu.getX() + 100, cpu.getY() + 100, archPanel.getMinX(),
