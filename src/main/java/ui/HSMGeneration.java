...
public class HSMGeneration implements Runnable {
        ...
        try {
            ...
            for (AvatarPragmaSecret pragma : confResults.keySet()) {
                if (confResults.get(pragma).isProved() && !confResults.get(pragma).isSatisfied()) {
                    ...
                    TraceManager.addDev(
                            pragma.getArg().getBlock().getName() + "." + pragma.getArg().getName() + " is not secret");
    ...
    public void run() {
        ...
        for (String cpuName : selectedCpuTasks.keySet()) {
            ...
            TMLCPrimitiveComponent hsm = new TMLCPrimitiveComponent(0, 500, tcdp.getMinX(), tcdp.getMaxX(),
                        ...
                        if (readChannel.getSecurityContext().equals("")) {
                            ...
                            if (fromStart != null) {
                                if (nonSecChans.contains(compName + "__" + readChannel.getChannelName() + "_chData")
                                        || nonAuthChans.contains(compName + "__" + readChannel.getChannelName())) {
                                    channelInstances.add(tg);
                                    HSMChannel ch = new HSMChannel(readChannel.getChannelName(), compName,
                                            HSMChannel.DEC);
            ...
            if (hsmChannels.keySet().size() > 3) {
                TMLADChoice choice2 = new TMLADChoice(xc, 400, tad.getMinX(), tad.getMaxX(), tad.getMinY(),
                        tad.getMaxY(), false, null, tad);
                int i = 0;
        ...
        // For all the tasks that receive encrypted data, decrypt it, assuming it has no
        // associated HSM
        for (TMLTask task : tmap.getTMLModeling().getTasks()) {
            ...
            for (String chan : secChannels.keySet()) {
                ...
                for (TGComponent chI : channelInstances) {
                    ...
                    int yShift = 100;
                    TMLADDecrypt dec = new TMLADDecrypt(xpos, ypos + yShift, tad.getMinX(), tad.getMaxX(),
                            tad.getMinY(), tad.getMaxY(), false, null, tad);
                    dec.securityContext = ch.securityContext;
                    ...
                    fromStart.setP2(dec.getTGConnectingPointAtIndex(0));
                    if (point != null) {
                        fromStart = new TGConnectorTMLAD(dec.getX(), dec.getY(), tad.getMinX(), tad.getMaxX(),
            ...
            for (String chan : secChannels.keySet()) {
                ...
                for (TGComponent tg : tad.getComponentList()) {
                    if (tg instanceof TMLADWriteChannel) {
                        TMLADWriteChannel writeChannel = (TMLADWriteChannel) tg;
                        if (writeChannel.getChannelName().equals(chan)
                ...
                for (TGComponent chI : channelInstances) {
                    ...
                    TMLADEncrypt enc = new TMLADEncrypt(xpos, ypos, tad.getMinX(), tad.getMaxX(), tad.getMinY(),
                            tad.getMaxY(), false, null, tad);
        ...
        for (String cpuName : selectedCpuTasks.keySet()) {
            ...
            // Add new memory
            TMLArchiMemoryNode mem = new TMLArchiMemoryNode(cpu.getX() + 100, cpu.getY() + 100, archPanel.getMinX(),
            ...
            TMLArchiHWANode hwa = new TMLArchiHWANode(cpu.getX() + 100, cpu.getY() + 100, archPanel.getMinX(),
                    archPanel.getMaxX(), archPanel.getMinY(), archPanel.getMaxY(), true, null, archPanel);
            archPanel.addComponent(hwa, cpu.getX() + 100, cpu.getY() + 100, false, true);
            hwa.setName("HSM_" + cpuName);
            // Add hsm task to hwa

            TMLArchiArtifact hsmArt = new TMLArchiArtifact(cpu.getX() + 100, cpu.getY() + 100, archPanel.getMinX(),
            ...
            TMLArchiBUSNode bus = new TMLArchiBUSNode(cpu.getX() + 100, cpu.getY() + 100, archPanel.getMinX(),
                    archPanel.getMaxX(), archPanel.getMinY(), archPanel.getMaxY(), true, null, archPanel);
            bus.setPrivacy(1);
            ...
            // Connect Bus and CPU
            TMLArchiConnectorNode connect = new TMLArchiConnectorNode(cpu.getX() + 100, cpu.getY() + 100,
