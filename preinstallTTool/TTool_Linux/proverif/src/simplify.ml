...
let rec copy_process add_in_glob_table = function
  ...
  | Restr(n,(args,env),p,occ) -> 
	...
	Restr(n', (update_args_opt args,update_env env), Reduction_helper.process_subst (copy_process add_in_glob_table p) n (FunApp(n',[])), new_occurrence())
      else
	Restr(n, (update_args_opt args,update_env env), copy_process add_in_glob_table p, new_occurrence())
...
let apply_test cond t1 t2 =
  ...
  | FunApp(test', [cond'; t1'; t2']) when test' == test && Terms.equal_terms t1' t2 ->
      FunApp(test, [FunApp(Terms.and_fun(), [cond; make_not cond']); t2'; t2])
  | _ -> 
      match t2 with
	FunApp(test', [cond'; t1'; t2']) when test' == test && Terms.equal_terms t1 t1' ->
	  FunApp(test, [FunApp(Terms.or_fun(), [cond; cond']); t1; t2'])
      |	FunApp(test', [cond'; t1'; t2']) when test' == test && Terms.equal_terms t1 t2' ->
	  FunApp(test, [FunApp(Terms.or_fun(), [cond; make_not cond']); t1; t1'])
      |	_ ->
	...
	with No_Merging ->
	  match p2 with
	    ...
	    (* The input process is equivalent to 
	       ...
	       Try merging q with some other process. *)
	      next_f ((make_not cond)::accu_cond) q (context p1) 
	  | _ ->
	    (* Look into the leaves of p2 *)
	      select_leaf_D ((make_not cond)::accu_cond) (fun p_hole -> context(IfNorm(cond, p1, p_hole))) next_f p2
      end
  | NormProc(p) ->
...
let rec and_list = function
    [] -> Terms.true_term
  | [a] -> a
  | a::l -> FunApp(Terms.and_fun(), [a; and_list l])
...
let rec or_not_list = function
    [] -> Terms.false_term
  | [a] -> make_not a
  | a::l -> FunApp(Terms.or_fun(), [make_not a; or_not_list l]) 
...
let or_and cond cond_list1 cond_list2 =
  ...
  let choice_true_false = FunApp(Param.choice_fun Param.bool_type, [Terms.true_term; Terms.false_term]) in
  if Terms.equal_terms cond choice_true_false then
    and_list ((FunApp(Param.choice_fun Param.bool_type, [and_list rest1; and_list rest2]))::common)
  else
    if rest1 = [] then
      if rest2 = [] then
	and_list common
      else
	and_list ((FunApp(Terms.or_fun(), [cond; and_list rest2]))::common)
    else
      if rest2 = [] then
	and_list ((FunApp(Terms.or_fun(), [make_not cond; and_list rest1]))::common)
      else
	let test = Terms.gtest_fun Param.bool_type in
	and_list ((FunApp(test, [cond; and_list rest1; and_list rest2]))::common)
...
let rec merge_2_D next_f cond p1 p2 = 
  try
	...
	select_leaf_D [] (fun p -> p) (fun cond_list q2 rest ->
	  merge_Q cond q1 q2 (fun q_proc -> next_f (IfNorm(FunApp(Terms.or_fun(), [cond; and_list cond_list]), NormProc(q_proc), rest)))) p2
    | _, NormProc(q2) ->
	(* Try merging q2 with one leaf of p1 *)
	select_leaf_D [] (fun p -> p) (fun cond_list q1 rest ->
	  merge_Q cond q1 q2 (fun q_proc -> next_f (IfNorm(FunApp(Terms.and_fun(), [cond; or_not_list cond_list]), rest, NormProc(q_proc))))) p1
    | _,_ -> 
