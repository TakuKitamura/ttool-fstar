...
//asyncchannel* __B_sendReq__B2_receiveReq* __attribute__((section("section_mwmr1"))) = ADDR;
...
if (pthread_mutex_init(&__mainMutex, NULL) < 0) { exit(-1);}
  ...
  //ajoute 18.02. s'occupe uniquement du mapping de staches sur les processeurs
  ...
  attr_t = malloc(sizeof(pthread_attr_t));
  attr_t->cpucount = 0;   
  pthread_create(&thread__B, attr_t, mainFunc__B1, (void *)"B");

  ptr =malloc(sizeof(pthread_t));
  thread__B2 = (pthread_t)ptr; 
  attr_t = malloc(sizeof(pthread_attr_t));
  attr_t->cpucount = 1;   
  pthread_create(&thread__B, attr_t, mainFunc__B2, (void *)"B");
