...
omp_uint_t
gomp_resolve_num_threads (omp_uint_t specified, omp_uint_t count)
{
  struct gomp_thread *thread = gomp_thread();
  struct gomp_task_icv *icv;
  omp_uint_t threads_requested, max_num_threads, num_threads;
  omp_ulong_t remaining;
  ...
  if (icv->dyn_var)
    {
      omp_uint_t dyn = gomp_dynamic_max_threads ();
...
void
GOMP_parallel_start (void (*fn) (void *), void *data, omp_uint_t num_threads)
{
...
omp_int_t
omp_get_ancestor_thread_num (omp_int_t level)
{
...
omp_int_t
omp_get_team_size (omp_int_t level)
{
  struct gomp_team_state *ts = &gomp_thread ()->ts;
  if (level < 0 || level > ts->level)
    return -1;
