...
void
gomp_barrier_init (gomp_barrier_t *bar, omp_uint_t count)
{
...
void
gomp_barrier_reinit (gomp_barrier_t *bar, omp_uint_t count)
{
  gomp_mutex_lock (&bar->mutex1);
  bar->total = count;
  gomp_mutex_unlock (&bar->mutex1);
...
void
gomp_team_barrier_wake (gomp_barrier_t *bar, omp_int_t count)
{
  if (count == 0)
    count = bar->total - 1;
  while (count-- > 0)
