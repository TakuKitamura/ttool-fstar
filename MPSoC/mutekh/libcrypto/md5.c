...
void md5_byte_reverse(unsigned char *buf, unsigned longs)
{
  uint32_t t;
 ...
 */
static void md5_transform(buf, in)
...
{
    struct crypto_md5_ctx_s *ctx = ctx_;
...
#endif
    memcpy(hash, ctx->buf, 16);

    memset(ctx, 0, sizeof(ctx));        /* In case it's sensitive */
}

struct crypto_hash_algo_s crypto_md5 = {
    .f_init = crypto_md5_init,
    .f_update = crypto_md5_update,
    .f_get = crypto_md5_get,
    .ctx_size = sizeof(struct crypto_md5_ctx_s),
    .hash_size = 16,
};
