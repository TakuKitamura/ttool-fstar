...
int main(void)
{
    ...
    if (scanf("%lf%ld%ld%ld%ld%ld%ld%ld%ld%lf",&TSTEP, &NMOL, &NSTEP, &NORDER,
            &NSAVE, &NRST, &NPRINT, &NFMC,&NumProcs, &CUTOFF) != 10) fprintf(stderr,"ERROR: Usage: water < infile, which must have 10 fields, see SPLASH documentation or comment at top of water.C\n");
#else
    TSTEP = 1.5e-16;
    NMOL  =  NMOLS;
    NSTEP =  3;
    ...
    { /* Do memory initializations */
        ...
        MAIN_INITENV((NumProcs),70000000,);  /* macro call to initialize
                                      shared memory etc. */
...
#ifdef DISTRIBUTED
static void allocate(void * START, unsigned int SIZE){
	void * temp;
                    ...
                    it simply calls the timestep routine */
{
    ...
    { long i,j,k;
	    ...
	    allocate(PFORCES[ProcID],(NMOL * sizeof (double **)));
	    //PFORCES[ProcID] = (double ***) G_MALLOC(NMOL * sizeof (double **));
	    for (j = 0; j < NMOL; j++) {
		    allocate(PFORCES[ProcID][j],(NDIR * sizeof (double *)));
		    //PFORCES[ProcID][j] = (double **) G_MALLOC(NDIR * sizeof (double *));
		    for (k = 0; k < NDIR; k++) {
			    allocate(PFORCES[ProcID][j][k],(NATOM * sizeof (double)));
			    //PFORCES[ProcID][j][k] = (double *) G_MALLOC(NATOM * sizeof (double));
		    }
	    }
    }
#endif
