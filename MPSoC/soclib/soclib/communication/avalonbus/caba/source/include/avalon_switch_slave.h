 ...
 * Maintainer: wagner
 * 
 * File : avalon_switch_slave.h
 * Date : 20/11/2008
 */


#ifndef SOCLIB_CABA_AVALON_SWITCH_SLAVE_H_
#define SOCLIB_CABA_AVALON_SWITCH_SLAVE_H_
...
namespace soclib { namespace caba {
    ...
    public:
      ...
      //       sc_out<sc_dt::sc_uint<avalon_param::Data_Width/8> >     writebyteenable;    //  avalon slave only; if used writedata must also be used. write and byteenable cannot be used
      //       sc_out<bool>                                            begintransfer;      //  avalon slave only
      ...
      //       sc_out<bool>                                            beginbursttransfer; //  avalon slave only

      //  FLOW CONTROL SIGNALS
      //       sc_in<bool>                                             readyfordata;       // 
...
#define ren(x) x(((std::string)(name_ + "_"#x)).c_str())
      ...
      AvalonSwitchSlave(std::string name_ = (std::string)sc_gen_unique_name("avalon_switch_slave"))
	: ren(clk),
	  ren(chipselect),
	  ren(address), 
	  ren(read),  
