 ...
 * Maintainers: nipo
 */
#ifndef SOCLIB_CABA_SIGNAL_VCI_MONITOR_H_
#define SOCLIB_CABA_SIGNAL_VCI_MONITOR_H_
...
#define __ren(x) x((name+"_" #x).c_str())
    VciMonitor(const std::string &name = sc_gen_unique_name("vci_monitor"))
		: __ren(rspack),
          __ren(rspval),
          __ren(rdata),
          __ren(reop),
...
#undef __ren
	...
	void operator()(VciMonitor<vci_param> &ports)
	{
		cmdack  (ports.cmdack);
...
#endif /* SOCLIB_CABA_SIGNAL_VCI_MONITOR_H_ */
