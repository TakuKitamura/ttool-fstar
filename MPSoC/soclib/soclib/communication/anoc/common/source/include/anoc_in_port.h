...
#ifndef _ANOC_IN_PORT_H_
#define _ANOC_IN_PORT_H_
...
//  - data of type anoc_target_port
//  - accept of type anoc_init_port,
// 
// This is just a facility to ease the binding at the netlist level
// => the binding must be done like : anoc_out_port.bind(anoc_in_port)
...
public:
  ...
  // port fields 
  anoc_target_port<anoc_data_transaction>  *data_port;
  anoc_init_port<anoc_accept_transaction>  *accept_port;
...
public:
  ...
  //----------------------
#ifdef TLM_TRANS_RECORD  
  anoc_in_port(std::string port_name_, const char *transrecord_name_) {
    transrecord = new anoc_transrecord(transrecord_name_);
    data_port   = new anoc_target_port<anoc_data_transaction>((port_name_ + "_data").c_str(),
							      transrecord);
    accept_port = new anoc_init_port<anoc_accept_transaction>((port_name_ + "_accept").c_str(),
							      transrecord);
  }
#else
  anoc_in_port(std::string port_name_) {
    data_port   = new anoc_target_port<anoc_data_transaction>((port_name_ + "_data").c_str());
    accept_port = new anoc_init_port<anoc_accept_transaction>((port_name_ + "_accept").c_str());
  }
#endif // TLM_TRANS_RECORD 
  ...
  //----------------------
  inline void slave_bind(tlm_blocking_put_if<anoc_data_transaction>& iface) {
    data_port->bind(iface);
  }

  //----------------------
  // FIXME hierachical port to port binding (src module to dst module)
  //----------------------
  void hbind(anoc_in_port *in_port) {
    data_port->bind(*in_port->data_port);
