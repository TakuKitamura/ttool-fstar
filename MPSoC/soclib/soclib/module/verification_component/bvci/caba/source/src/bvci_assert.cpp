...
#include "bvci_assert.h"

namespace soclib { namespace caba {

#define tmpl(t) template<typename vci_param> t BasicVciAssert<vci_param>
...
tmpl(void)::LockedAddress::add(typename vci_param::addr_t aAddress) {
...
tmpl(void)::LockedAddress::add(typename vci_param::addr_t aAddress, int plen) {
...
tmpl(void)::LockedAddress::remove(typename vci_param::addr_t aAddress) {
   AddressInterval aiInterval(aAddress);
   typename std::set<AddressInterval>::iterator iter = saiAddresses.find(aiInterval);
   if (iter != saiAddresses.end()) {
...
tmpl(void)::LockedAddress::remove(typename vci_param::addr_t aAddress, int plen) {
   AddressInterval aiInterval(aAddress, plen);
   typename std::set<AddressInterval>::iterator iter = saiAddresses.find(aiInterval);
   if (iter != saiAddresses.end()) {
      AddressInterval* paiOther = NULL;
...
tmpl(void)::acquireRequestCell() {
   if (m_request_cells == 0) {
      if (m_packet_clen > 0) {
         ++m_packet_clen;
         assume(p_vci.clen == m_packet_clen && m_packet_cmd == p_vci.cmd
      ...
      m_pending_packets.add(Packet(p_vci));
      if (m_packet_cmd == 3) { // Locked read
         if (m_packet_contig)
            m_locked_addresses.add(m_packet_address, m_packet_plen);
         else
            m_locked_addresses.add(m_packet_address);
      };
   }
   else {
...
tmpl(void)::acquireResponseCell() {
   if (m_response_cells == 0) {
      ...
      const Packet& packet = m_pending_packets.last();
      if (packet.cmd == 2) { // Write
         if (packet.contig)
            m_locked_addresses.remove(packet.address, packet.plen);
         else
            m_locked_addresses.remove(packet.address);
      };
   };
   ++m_response_cells;
   --m_pending_packets.last().length();
   if (p_vci.reop) {
      assume(m_pending_packets.last().length() == 0,
