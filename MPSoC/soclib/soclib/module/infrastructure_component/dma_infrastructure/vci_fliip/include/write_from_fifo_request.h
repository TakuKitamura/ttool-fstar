...
#ifndef WRITE_FROM_FIFO_REQUEST_H
#define WRITE_FROM_FIFO_REQUEST_H

#include <vci_initiator_fsm.h>

#define ON_READ_FIFO(x) \
(bool (soclib::caba::BaseModule::*)(typename vci_param::data_t&))(&SC_CURRENT_USER_MODULE::x)

namespace soclib { namespace caba {

template <typename vci_param>
class WriteFromFifoRequest : public VciInitiatorReq<vci_param> {

public:
	typedef bool (BaseModule::*on_read_func_t)(typename vci_param::data_t& data);
...
private:
	...
	BaseModule *m_on_read_module;
	on_read_func_t m_on_read;

public:

	WriteFromFifoRequest(unsigned int source, int srcMode, int len) {
	...
	void setOnRead(BaseModule *, on_read_func_t);
