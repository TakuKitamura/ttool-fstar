...
#include "../include/channel_estimation.h"
#include "soclib_endian.h"

namespace soclib { namespace caba {

#define tmpl(x) template<typename vci_param> x Channel_estimation<vci_param>

tmpl(void)::transition()
{
	if ( p_resetn.read() == false ) {
			m_state = FIFO_CHEST_READ;
			m_cycles_left = 64;
			m_ptr = 0;
			return;
	}
	...
	if ( m_cycles_left > 0 ) {
		switch (m_state) {
		case FIFO_CHEST_WAIT:
			--m_cycles_left;
			break;
		case FIFO_CHEST_READ:
	...
	if ( m_cycles_left == 0 ) {
		switch (m_state) {
		case FIFO_CHEST_WAIT:
			m_state = FIFO_CHEST_WRITE;
			m_cycles_left = sizeof(m_output_buffer)/sizeof(word_t);
			m_ptr = 0;
			break;
		case FIFO_CHEST_WRITE:
			m_state = FIFO_CHEST_READ;
			m_cycles_left = sizeof(m_input_buffer)/sizeof(word_t);
			m_ptr = 0;
			break;
		case FIFO_CHEST_READ:
			m_state = FIFO_CHEST_EXEC;
			m_cycles_left = 0;
			break;
		case FIFO_CHEST_EXEC:
			do_chest();
			m_state = FIFO_CHEST_WAIT;
...
tmpl(void)::genMoore()
{
	p_from_ctrl.r = (m_state == FIFO_CHEST_READ);
	p_to_ctrl.w = (m_state == FIFO_CHEST_WRITE);
	if (m_state == FIFO_CHEST_WRITE)
