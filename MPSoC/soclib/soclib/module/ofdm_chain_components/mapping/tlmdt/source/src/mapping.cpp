...
#define tmpl(x) x Mapping

////////////////////////////////////////////////////////////////////////////////////////////////////////
// CONSTRUCTOR
//////////////////////////////////////////////////////////////////////////////////////////////// ///////
tmpl(/**/)::Mapping(sc_core::sc_module_name name,
			uint32_t id,
	   ...
	   m_status_registers(n_status)
{
  ...
  for(uint32_t i=0;i<m_read_channels;i++){
    ...
    tmpName << "read_fifo" << i;
    p_read_fifo.push_back(new tlm_utils::simple_initiator_socket_tagged<Mapping,32,tlm::tlm_base_protocol_types>(tmpName.str().c_str()));
    p_read_fifo[i]->register_nb_transport_bw(this, &Mapping::readResponseReceived, i);
  }
  ...
  for(uint32_t i=0;i<m_write_channels;i++){
    ...
    p_write_fifo.push_back(new tlm_utils::simple_initiator_socket_tagged<Mapping,32,tlm::tlm_base_protocol_types>(tmpName.str().c_str()));
    p_write_fifo[i]->register_nb_transport_bw(this, &Mapping::writeResponseReceived, i);
  }
  ...
  for(uint32_t i=0;i<m_config_registers;i++){
    ...
    std::ostringstream tmpName;
    tmpName << "config" << i;
    p_config.push_back(new tlm_utils::simple_target_socket_tagged<Mapping,32,tlm::tlm_base_protocol_types>(tmpName.str().c_str()));
    p_config[i]->register_nb_transport_fw(this, &Mapping::writeConfigReceived, i);
  }
  ...
  for(uint32_t i=0;i<m_status_registers;i++){
    ...
    std::ostringstream tmpName;
    tmpName << "status" << i;
    p_status.push_back(new tlm_utils::simple_target_socket_tagged<Mapping,32,tlm::tlm_base_protocol_types>(tmpName.str().c_str()));
    p_status[i]->register_nb_transport_fw(this, &Mapping::readStatusReceived, i);
  ...
  while(true){
    count++;
	 ...
	 m_pdes_local_time->add(m_read_fifo_depth * UNIT_TIME);
      ...
      bits_index = 0;	
      for (i_symb=0; i_symb < m_read_fifo_depth/2 ; i_symb++)
     {
	      // Map I first
	      point=0;
	      for (i=0 ; i < m_I ; i++)
	      {
		  bit = (read_buffer[bits_index] & 0x1);
		  point = point ^ ( bit<<(m_I-1-i));
	      }
	      write_buffer[bits_index++] = constellation_I[point];
	      // Map Q last
	      point=0;
	      for (i=0 ; i < m_Q ; i++)
	      {
		  ...
		  point = point ^ ( bit<<(m_Q-1-i));
              ...
              write_buffer[bits_index++] = constellation_Q[point];
      }      
      // Writing the result on data
     for(uint32_t k=0, j=0; k < m_write_fifo_depth; k++, j+=vci_param::nbytes) {
          utoa(write_buffer[k], data, j);     
     }
