...
typedef struct {
  ...
  uint32_t is_internal:1;
  uint32_t date:6;
} __attribute__((packed)) papr_desc_slot_t;

typedef union {
  papr_desc_slot_t descripteur_slot;
  uint32_t binary[ROUDED_UP_DIV(sizeof(papr_desc_slot_t),sizeof(uint32_t))];
} papr_desc_slot_or_data_t;
