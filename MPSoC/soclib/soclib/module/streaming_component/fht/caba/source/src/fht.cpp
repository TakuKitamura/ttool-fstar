...
//--------------------------------------------------
#ifdef NC_SYSTEMC
NCSC_MODULE_EXPORT(fht)
#endif
#ifdef MENTORG
SC_MODULE_EXPORT(fht);
#endif
...
#define FHT_NB_FIFO_IN       2
#define FHT_SIZE_FIFO_IN0   32
#define FHT_SIZE_FIFO_IN1   32
#define FHT_NB_FIFO_OUT      2
#define FHT_SIZE_FIFO_OUT0  32
#define FHT_SIZE_FIFO_OUT1  32
#define FHT_NB_CFG_MONOCORE  1
#define FHT_SNO_NAME        "res_fht"

#define FHT_NB_CORES        1
#define FHT_NB_CONFIG_REGS  6
#define FHT_NB_STATUS_REGS  5
...
// -- constructor
//--------------------------------------------------
fht::fht(sc_module_name name, 
                           t_uint32 clk_period_):
                             sc_module(name),
                             clk_period(clk_period_),
                             p_clk("p_clk"),
                             p_resetn("p_resetn"),
                             p_from_MWMR(alloc_elems<FifoInput<uint32_t> >("p_from_MWMR", FHT_NB_FIFO_IN)),
                             p_to_MWMR(alloc_elems<FifoOutput<uint32_t>  >("p_to_MWMR", FHT_NB_FIFO_OUT)),
                             p_core_config("p_core_config"),
                             p_config(alloc_elems<sc_in<uint32_t>  >("p_config", FHT_NB_CONFIG_REGS)),
                             p_status(alloc_elems<sc_out<uint32_t> >("p_status", FHT_NB_STATUS_REGS))
{
  printf("Constructor of: fht\n");

  nb_cores    = FHT_NB_CORES;
  nb_fifo_in  = FHT_NB_FIFO_IN;
  nb_fifo_out = FHT_NB_FIFO_OUT;
  ...
  // Resource creation
  res_wrapper = new anoc_copro_wrapper("res_wrapper", nb_cores, nb_fifo_in, nb_fifo_out, FHT_NB_CONFIG_REGS, FHT_NB_STATUS_REGS, clk_period);
  ...
  for (int i=0; i< FHT_NB_CONFIG_REGS; i++) {
    res_wrapper->p_config[i](p_config[i]);
  }
  for (int i=0; i< FHT_NB_STATUS_REGS; i++) {
    res_wrapper->p_status[i](p_status[i]);
  }
  // message
  EXPRINT(top, 0,"fht created\n");

} // end


// Bind ports to core
void fht::bind_cores_to_wrapper(core_base **cores) {
...
void fht::get_size_of_fifos(t_uint16* size_fifo_in,t_uint16* size_fifo_out) {
#ifdef FHT_SIZE_FIFO_IN0
  size_fifo_in[0] = FHT_SIZE_FIFO_IN0;
#endif
#ifdef FHT_SIZE_FIFO_IN1
  size_fifo_in[1] = FHT_SIZE_FIFO_IN1;
#endif
#ifdef FHT_SIZE_FIFO_IN2
  size_fifo_in[2] = FHT_SIZE_FIFO_IN2;
#endif
#ifdef FHT_SIZE_FIFO_IN3
  size_fifo_in[3] = FHT_SIZE_FIFO_IN3;
#endif
#ifdef FHT_SIZE_FIFO_OUT0
  size_fifo_out[0] = FHT_SIZE_FIFO_OUT0;
#endif
#ifdef FHT_SIZE_FIFO_OUT1
  size_fifo_out[1] = FHT_SIZE_FIFO_OUT1;
#endif
#ifdef FHT_SIZE_FIFO_OUT2
  size_fifo_out[2] = FHT_SIZE_FIFO_OUT2;
#endif
#ifdef FHT_SIZE_FIFO_OUT3
  size_fifo_out[3] = FHT_SIZE_FIFO_OUT3;
#endif
