...
void *mainFunc__DrivingPowerReductionStrategy(struct mwmr_s *channels_DrivingPowerReductionStrategy[]){
  ...
  while(__currentState != STATE__STOP__STATE) {
    switch(__currentState) {
      ...
      waitFor((minReducePowerTime)*1000000, (maxReducePowerTime)*1000000);
      ...
      __params0[0] = &value;
      makeNewRequest(&__req0, 586, RECEIVE_SYNC_REQUEST, 0, 0, 0, 1, __params0);
      __req0.syncChannel = &__DangerAvoidanceStrategy_reducePower__DrivingPowerReductionStrategy_getReducePowerOrder;
