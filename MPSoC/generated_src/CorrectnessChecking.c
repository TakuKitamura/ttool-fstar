...
void *mainFunc__CorrectnessChecking(struct mwmr_s *channels_CorrectnessChecking[]){
  ...
  while(__currentState != STATE__STOP__STATE) {
    switch(__currentState) {
      ...
      if (canHaveInvalid) {
        makeNewRequest(&__req0, 322, IMMEDIATE, 0, 0, 0, 0, __params0);
        addRequestToList(&__list, &__req0);
      }
      makeNewRequest(&__req1, 332, IMMEDIATE, 0, 0, 0, 0, __params1);
      ...
      traceRequest(__myname, __returnRequest);
       if (__returnRequest == &__req0) {
        ...
        makeNewRequest(&__req0, 317, SEND_SYNC_REQUEST, 0, 0, 0, 2, __params0);
      ...
      __params0[0] = &id;
      __params0[1] = &position;
      makeNewRequest(&__req0, 319, RECEIVE_SYNC_REQUEST, 0, 0, 0, 2, __params0);
