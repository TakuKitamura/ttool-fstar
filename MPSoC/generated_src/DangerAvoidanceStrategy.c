...
void *mainFunc__DangerAvoidanceStrategy(struct mwmr_s *channels_DangerAvoidanceStrategy[]){
  ...
  while(__currentState != STATE__STOP__STATE) {
    switch(__currentState) {
      ...
      makeNewRequest(&__req0, 622, SEND_SYNC_REQUEST, 0, 0, 0, 2, __params0);
      ...
      traceVariableModification("DangerAvoidanceStrategy", "value", value,0);
      __params0[0] = &value;
      makeNewRequest(&__req0, 623, SEND_SYNC_REQUEST, 0, 0, 0, 1, __params0);
      ...
      if (!(speed>5)) {
        makeNewRequest(&__req0, 645, IMMEDIATE, 0, 0, 0, 0, __params0);
        addRequestToList(&__list, &__req0);
      }
      if (speed>5) {
        __params1[0] = &value;
        makeNewRequest(&__req1, 626, SEND_SYNC_REQUEST, 0, 0, 0, 1, __params1);
        __req1.syncChannel = &__DangerAvoidanceStrategy_reducePower__DrivingPowerReductionStrategy_getReducePowerOrder;
      ...
      makeNewRequest(&__req0, 628, RECEIVE_SYNC_REQUEST, 0, 0, 0, 3, __params0);
