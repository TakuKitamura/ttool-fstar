...
/* Only processor 0 starts execution on reset */
#if defined(CONFIG_GDB_SERVER)
...
template <class Iss>
NEW_CPU(new_cpu){
return new caba::VciXcacheWrapper<vci_param, ISS_NEST(Iss)>(e->name.c_str(), e->id, maptab, IntTab(e->id),8,1,4,8,1,4);
}
...
maptab.add(Segment("cram0", 0x10000000, 0x1000000, IntTab(2), true));
maptab.add(Segment("uram0", 0x11200000, 0x1000000, IntTab(2), false));
...
bool heterogeneous = (argc > 2);
  ...
  for (int i = 0; i < (argc - 1); i += 2){
    ...
    if (heterogeneous) {
	 ...
	 if (i == 0)
	    data_ldr.load_file(std::string(kernel_p) + ";.data;.channel0;.channel1;.channel2;.channel3;.channel4;.channel5;.channel6;.channel7;.channel8;.channel9;.channel10;.channel11;.channel12;.channel13;.channel14;.cpudata;.contextdata");
      } else {
...
soclib::caba::VciVgmn<vci_param> vgmn("ICN0" , maptab, cpus.size()+3,12,10,8);
...
vcixicu.p_irq[i](cpus[i]->irq_sig[0]);
 ...
 vgmn.p_clk(signal_clk);
  ...
  vgmn.p_to_target[3](signal_vci_vcisimhelper);

  vgmn.p_to_target[4](signal_vci_xicu);
  vgmn.p_to_target[5](signal_vci_vcirttimer);

  vgmn.p_to_target[6](signal_vci_vcifdtrom);
...
vgmn.p_to_target[2](signal_vci_vciram0);

vgmn.p_to_target[8](signal_vci_vcifdaccesst);
vgmn.p_to_target[9](signal_vci_ethernett);
vgmn.p_to_target[10](signal_vci_bdt);
vgmn.p_to_target[11](signal_vci_vcilocks);
...
vgmn.p_to_target[7](signal_vci_tty0);
